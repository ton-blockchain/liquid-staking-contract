...
export const promptBool    = async (prompt:string, options:[string, string], ui:UIProvider) => {
    ...
    do {
        let res = (await ui.input(`${prompt} ${options.join('/')} :`)).toLowerCase();
        yes = res == opts[0]
        if(!yes)
            no  = res == opts[1];
    } while(!(yes || no));
...
export const promptMulti = async (prompt:string, options: [string, string, ...string[]], ui: UIProvider) => {
    const lowerOpts = options.map(o => o.toLowerCase());
    let resIdx : number;
    do {
        let res = (await ui.input(`${prompt} ${options.join('/')} >`)).toLowerCase();
        resIdx  = lowerOpts.findIndex(x => x.startsWith(res));
        if(resIdx < 0 || res == '') {
            ui.write("Your input doesn't match any provided options\n");
        }
        else {
            return resIdx;
        }
...
export const chooseBool = async (prompt: string, true_opt:string, false_opt: string, ui: UIProvider) => {
    let res = await ui.choose(prompt, [true_opt, false_opt], (c) => c);
    return res == true_opt;
}
...
export const promptAmount = async (prompt:string, provider:UIProvider, isInt: boolean = false, fallback?: number) => {
    let resAmount:number;
    do {
        const hasFallback = fallback !== undefined;
        const printPrompt = hasFallback ? `${prompt} (default: ${fallback})` : prompt;

        let inputAmount   = await provider.input(printPrompt);

        if(inputAmount == '' && hasFallback) {
            resAmount = fallback;
        }
        else {
        ...
        if(isNaN(resAmount)) {
            provider.write("Failed to convert " + inputAmount + " to number");
        }
        else if(resAmount < 0) {
            provider.write(`Unsigned number expected`);
        }
        else if(isInt) {
            if(! Number.isInteger(resAmount)) {
                provider.write(`${inputAmount} is not integer!`);
            }
            else {
                return resAmount;
            }
            ...
            return Number(resAmount.toFixed(9));
        }
    } while(true);
}

export const promptAmountBigInt = async (prompt:string, provider:UIProvider) => {
    ...
    do {
        ...
        try {
            ...
            provider.write("Failed to convert " + inputAmount + " to BigInt:" + (err as Error).toString());
        }
    } while(true);
}
...
export const waitForTransaction = async (provider:NetworkProvider, address:Address,
    ...
    do {
        ...
        let newBlockNum = await getLastBlock(provider);
        if (blockNum == newBlockNum) {
            continue;
        }
        blockNum = newBlockNum;
        const curState = await provider.api().getAccountLite(blockNum, address);
